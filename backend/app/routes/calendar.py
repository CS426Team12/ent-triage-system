...
@router.post("/physicians/{physician_id}/create", status_code=status.HTTP_201_CREATED)
def create_physician_calendar(
    ...
    logger.info(
        f"POST /calendar/physicians/{physician_id}/create - user: {current_user.email}"
    )
    ...
    try:
        ...
        try:
            audit_meta = (
                get_audit_meta(request) if request is not None else {"ip": None}
            )
        ...
        db.rollback()
        logger.exception(
            f"POST /calendar/physicians/{physician_id}/create - Error: {str(e)}"
        )
        raise HTTPException(
            status_code=500, detail="Failed to create physician calendar"
        )


@router.post("/physicians/{physician_id}/attach", status_code=status.HTTP_201_CREATED)
def attach_physician_calendar(
    ...
    logger.info(
        f"POST /calendar/physicians/{physician_id}/attach - user: {current_user.email}"
    )
    ...
    try:
        ...
        try:
            calendar_service.calendarList().insert(
                body={"id": payload.calendarID}
            ).execute()
        except Exception as e:
            logger.exception(
        ...
        try:
            ...
            logger.warning(
                f"POST /calendar/physicians/{physician_id}/attach - could not set calendar color"
            )
...
def update_calendar_color(
    ...
    logger.info(
        f"PATCH /calendar/physicians/{physician_id}/color - user: {current_user.email}"
    )
...
def get_availability(
    ...
    logger.info(
        f"GET /appointments/availability - physicianID: {physicianID}, date: {date}, user: {current_user.email}"
    )
    ...
    try:
        ...
        time_min = (
            datetime.fromisoformat(f"{date}T08:00:00")
            .replace(tzinfo=CLINIC_TZ)
            .isoformat()
        )
        time_max = (
            datetime.fromisoformat(f"{date}T17:00:00")
            .replace(tzinfo=CLINIC_TZ)
            .isoformat()
        )
...
def list_appointments(
    ...
    logger.info(
        f"GET /appointments - caseID: {caseID}, physicianID: {physicianID}, status: {status}, user: {current_user.email}"
    )
    ...
    try:
        ...
        return {
            **appointment.model_dump(),
            "physicianName": (
                f"Dr. {physician.firstName} {physician.lastName}" if physician else None
            ),
        }
...
def create_appointment(
    ...
    logger.info(
        f"POST /appointments - caseID: {payload.caseID}, physicianID: {payload.physicianID}, user: {current_user.email}"
    )
    ...
    try:
        ...
        patient_name = (
            f"{patient.firstName} {patient.lastName}" if patient else "Patient"
        )
...
def reschedule_appointment(
    ...
    logger.info(
        f"PATCH /appointments/{appointment_id} - body: {payload.model_dump(exclude_unset=True)}, user: {current_user.email}"
    )
    ...
    try:
        ...
        new_start = normalize_to_clinic_tz(
            payload.scheduledAt or appointment.scheduledAt
        )
        new_end = normalize_to_clinic_tz(
            payload.scheduledEnd or appointment.scheduledEnd
        )
